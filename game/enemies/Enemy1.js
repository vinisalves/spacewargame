...
export default class Enemy1 {
  constructor(ctx) {
    ...
    this.sprite.src = "game/assets/img/enemies/enemy1.png";
    (this.x = Math.random() * GAME_CONFIG.width), (this.y = -300);
    this.speed = 6;
    ...
    this.life = 100;
    this.direction =
      GAME_CONFIG.player.aircraft.x > this.x
        ? (this.direction = 1)
        : (this.direction = -1);
  ...
  fire() {
    if (GAME_CONFIG.player.aircraft.x > this.x) {
      enemiesProjectiles.push(
        new Projectile(this.ctx, this.x + 20, this.y + this.height - 80, 1)
      );
    } else {
      enemiesProjectiles.push(
        new Projectile(this.ctx, this.x + 20, this.y + this.height - 80, -1)
...
class Projectile {
  constructor(ctx, x, y, direction) {
    ...
    if (!soundController.ENEMY1_FIRE.muted) {
      const clone = soundController.ENEMY1_FIRE.cloneNode();
      clone.volume = soundController.ENEMY1_FIRE.volume;
      clone.play();
    }
  }
