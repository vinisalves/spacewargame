...
export default class Enemy2 {
    constructor(ctx) {
        ...
        this.max_y = Math.floor(Math.random() * (GAME_CONFIG.height - this.height) / 2);

    }

    fire() {
        this.totalProjectiles--;
        enemiesProjectiles.push(new Projectile(this.ctx, this.x + 20, this.y + this.height - 80));
    }
    ...
    draw() {
        ...
        if (GAME_CONFIG.frame % this.randomize === 0 && this.y + this.height > 0 && this.totalProjectiles > 0) this.fire();
